...
const Board = () => {
  ...
  const fetchUserDetails = async () => {
    try {
      const response = await axios.get(`/user/${userEmail}`); // Assuming your backend is running on the same host
      setUser(response.data); // Set user details in state
    } catch (err) {
      if (err.response && err.response.data) {
        setError(err.response.data.msg); // Set error message if available
      } else {
  ...
  const handleLogout = () => {
    // Perform logout action (clear session data, etc.)
    ...
    <div className="container">
      ...
      <div className="content">
        ...
        <div className="profile">
            <button onClick={fetchUserDetails}>profile</button>
        {user && (
        <div>
